...
export function buildActions(config: ServiceNowPluginConfig): Record<string, ActionDefinition> {
   ...
   */
  function portal(env: string): string {
  ...
  return {
    ...
    openRemoteInstance: {
      ...
      resolve: ({ env, targetEnv }) =>
        `${baseUrl(env)}/sys_remote_update_set.do?sysparm_query=type=${targetEnv}^active=true`,
    },
    ...
    openCurrentUpdateSet: {
      ...
      resolve: ({ env }) =>
        `${baseUrl(env)}/sys_update_set.do?sysparm_query=sys_id=javascript:gs.getPreference('sys_update_set')`,
    },
    ...
    searchTable: {
      ...
      resolve: ({ env, table, text }) =>
        `${baseUrl(env)}/${resolveTable(table)}_list.do?sysparm_search=${encodeURIComponent(text)}`,
    },
    ...
    searchTableByQuery: {
      ...
      resolve: ({ env, table, query }) =>
        `${baseUrl(env)}/${resolveTable(table)}_list.do?sysparm_query=${encodeURIComponent(query)}`,
    },
    ...
    searchTableByFilter: {
      ...
      resolve: ({ env, table, filter }) =>
        `${baseUrl(env)}/${resolveTable(table)}_list.do?sysparm_query=${encodeURIComponent(resolveFilter(filter))}`,
    },
  };
}
